...
| Change fonts | 🎨 Theme → heading_font / body_font / accent_font | Pick from dropdown, OR type any Google Font name (e.g. `Lobster`) |
| Use a non-Google font | 🎨 Theme → custom_font_css + the font field above | Paste @import URL from FontShare/Adobe/wherever, then reference family name in font field. See EDITOR_GUIDE.md for examples. |
| Use Comico | 🎨 Theme → heading_font (or body_font, accent_font) → type `Comico` | Comico is pre-installed; just type the name. |
